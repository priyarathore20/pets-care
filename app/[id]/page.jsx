...
const PetForPublic = () => {
    ...
    if (id) {
      const fetchPetDetails = async () => {
        try {
          ...
          setData(res?.data);
          setLoading(false);
          showSnackbar('Pet fetched successfully', 'success');
        } catch (error) {
          setLoading(true);
      ...
      setLoading(true);
      showSnackbar('Could not find pet details', 'error');
    }
  }, [id]);
  ...
  return (
    <div className="flex flex-col bg-gray-100 px-20 py-10 w-screen h-4/5 overflow-hidden">
      {loading ? (
        <div className='w-[85vw] h-[88vh] overflow-hidden'>
          <Loader />
        </div>
      ) : (
        <>
          ...
          <div>
            ...
            {ownerDetails.map((item, id) => (
              <div
                key={id}
                className="flex justify-between mb-3 w-full md:w-[40%]"
              >
